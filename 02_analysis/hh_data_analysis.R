...
hh_scoring_method_1 <- df %>%
  ...
  slice_max(order_by = severity,
            prop = 0.25,
            with_ties = FALSE) %>%
  summarize(mean_max_25 = mean(severity),
...
hh_scoring_method_2 <- df %>%
  ...
  slice_max(order_by = severity,
            prop = 0.5,
            with_ties = FALSE) %>%
  summarize(mean_max_50 = mean(severity),
...
hh_scoring_method_3 <- df %>%
  ...
  slice_max(order_by = severity,
            prop = 0.75,
            with_ties = FALSE) %>%
  summarize(mean_max_75 = mean(severity),
            median_max_75 = median(severity))

hh_scoring_method_4 <- df %>%
  group_by(hh_id) %>%
  summarize(mean_all = mean(severity),
...
hh_scoring_method_6 <- df %>%
  group_by(hh_id) %>%
  summarize(any_indicator_inneed = sum(severity >= 3, na.rm = T)) %>%
  mutate(
...
df_irq_hno_pin <- df_irq_ocha_pin %>%
  ...
  mutate(aggregation_method = "HNO 2022 Intersectoral PiN",
...
df_irq_cluster_pin <- df_irq_ocha_pin %>%
  ...
  mutate(aggregation_method = "Max of Cluster calculated PiN",
...
bar_chart <-
  ...
  scale_fill_manual(values = c("#FFE0B2", "#00E676", "#D1C4E9", "#2154FF", "#B00020")) +
  theme(
